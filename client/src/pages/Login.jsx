...
import { Mail, Lock, ArrowRight, Loader2, AlertCircle, Zap } from 'lucide-react';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!email || !password) {
      dispatch(loginFailure('Identity and access key required'));
      return;
    }
    ...
    try {
      ...
      toast.success('Access authorized. Welcome to the Nexus.');
        ...
        <div className="relative z-10 space-y-12 max-w-lg">
          ...
          <div className="space-y-6">
            <h2 className="text-5xl font-bold tracking-tight leading-[1.1]">
              Accelerate Your <br />
              <span className="text-emerald-500 underline decoration-emerald-500/30 underline-offset-8">Academic Edge</span>
            </h2>
            <p className="text-lg text-slate-400 font-medium leading-relaxed">
              Connect to the ultimate AI academic partner. Process, analyze, and master any document with the world's most advanced learning assistant.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 pt-10">
            {[
              ...
              { label: 'Protocol', value: 'Nexus v4' }
            ].map(stat => (
              <div key={stat.label} className="p-4 bg-slate-900/40 border border-slate-800/50 rounded-2xl shadow-inner">
                <p className="text-[9px] uppercase font-bold text-slate-500 tracking-widest leading-none mb-2">{stat.label}</p>
                <p className="text-xs font-bold text-emerald-500 tracking-tight">{stat.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 md:p-16 relative">
        ...
        <div className="w-full max-w-md space-y-10 relative z-10 animate-in fade-in slide-in-from-right-8 duration-700">
          <div className="space-y-2">
            <h2 className="text-4xl font-bold text-white tracking-tight uppercase">Resume Access</h2>
            <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Secure authentication required for workspace entry</p>
          </div>
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 p-10 rounded-4xl shadow-2xl backdrop-blur-xl relative overflow-hidden group">
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="relative group/input">
                    ...
                    <input
                      ...
                      className="block w-full pl-14 pr-6 py-5 bg-slate-950/50 border border-slate-800/80 rounded-2xl text-slate-100 placeholder-slate-700 focus:outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500/50 transition-all font-medium text-sm"
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between px-1">
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Access Key</label>
                    <a href="#" className="text-[10px] text-emerald-500 hover:text-emerald-400 font-bold uppercase tracking-widest transition-colors">Lost Key?</a>
                  </div>
                  <div className="relative group/input">
            ...
            <div className="mt-12 pt-8 border-t border-slate-800/50 text-center">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">No nexus access yet?</span>{' '}
              <Link to="/register" className="text-xs font-bold text-emerald-500 hover:text-emerald-400 uppercase tracking-widest transition-colors ml-2 underline underline-offset-4 decoration-emerald-500/20 hover:decoration-emerald-500/50">Initialize ID</Link>
            </div>
          </div>
        </div>
      </div>
