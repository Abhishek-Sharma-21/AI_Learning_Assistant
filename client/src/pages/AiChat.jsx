...
import {
  MessageSquare, Send, ChevronDown, BookOpen,
  Bot, User, Loader2, Sparkles, RotateCcw, Lock, Zap
} from 'lucide-react';
...
const MsgMarkdown = ({ content }) => (
  <ReactMarkdown
    components={{
      ...
      code:       ({ children }) => <code className="px-1.5 py-0.5 rounded bg-slate-700 text-emerald-300 text-xs font-mono">{children}</code>,
      pre:        ({ children }) => <pre className="my-2 p-3 bg-slate-800 rounded-xl overflow-x-auto text-xs font-mono text-slate-200">{children}</pre>,
      blockquote: ({ children }) => <blockquote className="border-l-2 border-emerald-500 pl-3 my-2 text-slate-400 italic text-sm">{children}</blockquote>,
    }}
  >
    {content}
  </ReactMarkdown>
...
const AiChat = () => {
  ...
  const handleSelectDoc = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/documents/${id}/chat`, {
        headers: {
  ...
  const sendMessage = async (text) => {
    ...
    if (atChatLimit) { setShowUpgrade(true); return; }
    setInput('');
    ...
    try {
      const res  = await fetch(`http://localhost:5000/api/documents/${selectedDocId}/chat`, {
        method:  'POST',
        headers: { 
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: '⚠️ Connection error. Please check the server and try again.' }]);
    } finally {
      setChatLoading(false);
      inputRef.current?.focus();
    }
      ...
      <div className="shrink-0 flex items-center gap-4 px-5 py-3 border-b border-slate-800 bg-[#0f172a]/80 backdrop-blur-sm">
        ...
        {isFree && selectedDocId && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-800 border border-slate-700 rounded-xl">
            <MessageSquare size={12} className={atChatLimit ? 'text-red-400' : 'text-slate-400'} />
            <span className={`text-[11px] font-semibold ${atChatLimit ? 'text-red-400' : 'text-slate-400'}`}>
              {userMsgCount}/{FREE_CHAT_LIMIT}
            </span>
            {!atChatLimit && <span className="text-[10px] text-slate-600">free</span>}
            {atChatLimit && (
              <button onClick={() => setShowUpgrade(true)} className="text-[10px] font-bold text-emerald-400 ml-1 hover:text-emerald-300">
                Upgrade
              </button>
            )}
          </div>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-72 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl shadow-slate-950/60 max-h-72 overflow-y-auto z-20">
              {docsLoading ? (
                ...
                <p className="text-center py-8 text-sm text-slate-500">No documents found. Upload one first.</p>
              ) : (
                <div className="p-2 space-y-1">
                  {docs.map((doc) => (
                    ...
                    >
                      <BookOpen size={14} className="mt-0.5 shrink-0 text-slate-500" />
                      <span className="text-xs font-medium leading-relaxed">{doc.title}</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        {messages.length > 0 && (
          <button onClick={clearChat} title="Clear chat" className="p-2 text-slate-500 hover:text-slate-300 hover:bg-slate-800 rounded-lg transition-colors">
            <RotateCcw size={14} />
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        {!selectedDocId ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-5 text-center">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Select a document to start</h2>
              <p className="text-sm text-slate-500 mt-1 max-w-xs">Choose any document from the dropdown above and ask the AI anything about it.</p>
            </div>
            {/* Suggestion chips */}
            <div className="flex flex-wrap justify-center gap-2 mt-2 max-w-lg">
              {SUGGESTIONS.map((s) => (
                <button key={s} disabled className="px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-full text-xs text-slate-500 cursor-default">
                  {s}
                </button>
              ))}
            </div>
      ...
      {atChatLimit ? (
        <div className="shrink-0 px-4 pb-5 pt-3 border-t border-slate-800 bg-[#0f172a]/60 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-3 p-5 bg-amber-500/5 border border-amber-500/20 rounded-2xl">
            ...
            <p className="text-xs text-slate-400 text-center">You've used all {FREE_CHAT_LIMIT} free messages for this session.</p>
            <button
        ...
        <div className="shrink-0 px-4 pb-5 pt-3 border-t border-slate-800 bg-[#0f172a]/60 backdrop-blur-sm">
          <div className={`flex items-end gap-3 bg-slate-800/80 border rounded-2xl px-4 py-3 transition-colors ${selectedDocId ? 'border-slate-700 focus-within:border-emerald-500/50' : 'border-slate-800 opacity-50'}`}>
            <textarea
              ref={inputRef}
              rows={1}
              ...
              placeholder={selectedDocId ? 'Ask anything about this document…' : 'Select a document first'}
          ...
          <p className="text-center text-[10px] text-slate-600 mt-2">AI responses are based on document content only · Press Enter to send</p>
        </div>
      )}
    </div>
    </>
